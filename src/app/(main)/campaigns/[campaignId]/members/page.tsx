...
export default function CampaignMembersPage() {
  ...
  const [selectedRole, setSelectedRole] = useState<CampaignMemberRole>('BUZZER')
  ...
  const membersQuery = useQuery({ queryKey: ['campaign-members', campaignId], queryFn: () => campaignMembersApi.list(campaignId) })
  const usersQuery = useQuery({ queryKey: ['campaign-members-users'], queryFn: () => usersApi.list({ limit: 100, status: 'ACTIVE' }) })

  const members = membersQuery.data?.data ?? []
  const existingUserIds = new Set(members.map(member => member.userId))
  const selectableUsers = (usersQuery.data?.items ?? []).filter(user => !existingUserIds.has(user.id))

  const addMutation = useMutation({
    mutationFn: () => campaignMembersApi.add(campaignId, { members: [{ userId: selectedUserId, memberRole: selectedRole }] }),
    onSuccess: () => {
      ...
      <CampaignShell campaign={campaignQuery.data} campaignId={campaignId}>
        <div className="section-heading-row">
          <div>
            ...
            <p className="section-subtitle">Kelola member campaign agar Buzzer dapat melihat Blast Queue dan menerima CommentTask.</p>
          </div>
        </div>
        ...
        {(campaignQuery.isError || membersQuery.isError) && (
          <div className="blast-info-banner" style={{ marginBottom: '1.25rem', borderColor: 'rgba(239,68,68,0.35)' }}>
            ...
            <span>{getErrorMessage(campaignQuery.error ?? membersQuery.error, 'Gagal memuat campaign members.')}</span>
          </div>
        )}

        <div className="campaign-summary-panel" style={{ marginBottom: '1.25rem' }}>
          <div style={{ display: 'flex', alignItems: 'end', gap: '0.75rem', flexWrap: 'wrap' }}>
            <div className="form-group" style={{ minWidth: 260, marginBottom: 0 }}>
              ...
              <select className="input-field" value={selectedUserId} onChange={event => setSelectedUserId(event.target.value)} disabled={usersQuery.isLoading}>
                <option value="">{usersQuery.isLoading ? 'Loading users...' : 'Select active user'}</option>
            ...
            <div className="form-group" style={{ minWidth: 160, marginBottom: 0 }}>
              ...
              <select className="input-field" value={selectedRole} onChange={event => setSelectedRole(event.target.value as CampaignMemberRole)}>
                <option value="ADMIN">Admin</option>
                <option value="BUZZER">Buzzer</option>
                <option value="VIEWER">Viewer</option>
              </select>
            </div>
            <button type="button" className="btn-primary" disabled={!selectedUserId || addMutation.isPending} onClick={() => addMutation.mutate()}>
              <Plus size={14} />
        ...
        {membersQuery.isLoading ? (
          <div className="campaign-summary-panel">
            ...
            <p className="muted-meta" style={{ marginTop: '0.5rem' }}>Mengambil daftar member campaign.</p>
          </div>
        ) : !members.length ? (
          <EmptyState icon={<Users size={48} />} title="Belum ada member" description="Tambahkan member campaign agar Buzzer dapat melihat blast queue dan menerima comment task." />
        ) : (
          <CampaignMembersTable members={members} onRemove={member => removeMutation.mutate(member)} removingUserId={removingUserId} />
        )}
      </CampaignShell>
    </RoleGuard>
  )
