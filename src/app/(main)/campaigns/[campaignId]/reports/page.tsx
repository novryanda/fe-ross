...
export default function CampaignReportsPage() {
  ...
  const reportsQuery = useQuery({
    queryKey: ['campaign-reports', campaignId, reportKind],
    queryFn: () => reportsApi.listCampaignReports(campaignId, {}, reportKind),
  })
  ...
  const totalEngagement = blastReports.reduce((sum, r) => sum + calcEngagement(r), 0)
    ...
    <CampaignShell campaign={campaign} campaignId={campaignId}>
      <div className="section-heading-row">
        <div>
          ...
          <p className="section-subtitle">Blast reports dan Comment proofs yang disubmit untuk campaign ini.</p>
        </div>
        {isAdmin && (
      ...
      <div className="kpi-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
        <div className="kpi-v2" style={{ borderLeftColor: 'var(--status-completed)' }}>
          <div className="kpi-v2-icon" style={{ background: 'var(--status-completed-bg)', color: 'var(--status-completed)' }}><FileText size={20} /></div>
          <div><div className="kpi-v2-label">Blast Reports</div><div className="kpi-v2-value">{formatNumber(totalReports)}</div></div>
        </div>
        <div className="kpi-v2" style={{ borderLeftColor: 'var(--cyan)' }}>
          <div className="kpi-v2-icon" style={{ background: 'var(--cyan-dim)', color: 'var(--cyan)' }}><Eye size={20} /></div>
          <div><div className="kpi-v2-label">Total Views</div><div className="kpi-v2-value">{formatNumber(totalViews)}</div></div>
        </div>
        <div className="kpi-v2" style={{ borderLeftColor: '#f43f5e' }}>
          ...
          <div><div className="kpi-v2-label">Total Likes</div><div className="kpi-v2-value">{formatNumber(totalLikes)}</div></div>
        </div>
        <div className="kpi-v2" style={{ borderLeftColor: 'var(--status-active)' }}>
          ...
          <div><div className="kpi-v2-label">Total Engagement</div><div className="kpi-v2-value">{formatNumber(totalEngagement)}</div></div>
        </div>
      </div>
        ...
        filters={[
          {
            ...
            options: REPORT_KIND_FILTER.map((o) => ({ value: o.value, label: o.label })),
