...
export default function BlastTargetDetailPage() {
  ...
  const {
    ...
    queryKey: ["blast-target", campaignId, targetId],
    queryFn: () => blastApi.getTarget(campaignId, targetId),
  });
  const { data: attempts, isLoading: attemptsLoading } = useQuery({
    queryKey: ["blast-attempts", campaignId, targetId],
    queryFn: () => blastApi.getAttemptsByTarget(campaignId, targetId),
    enabled: isAdmin,
  });
  const { data: campaignReports } = useQuery({
    queryKey: ["campaign-reports", campaignId, "target", targetId],
    queryFn: () => blastApi.listReports(campaignId, { limit: 100 }),
  });
  ...
  const reblastMutation = useMutation({
    ...
    onSuccess: (newAttempt) => {
      toast.success(
        `Reblast attempt #${newAttempt.attemptNo} berhasil dibuat.`,
      );
  ...
  const statusMutation = useMutation({
    mutationFn: (status: BlastTargetStatus) =>
      blastApi.updateTargetStatus(campaignId, targetId, status),
    onMutate: async (status) => {
      ...
      const previousTarget = queryClient.getQueryData<BlastTarget>([
        "blast-target",
        campaignId,
        targetId,
      ]);
      ...
      queryClient.setQueryData<BlastTarget>(
        ["blast-target", campaignId, targetId],
        (old) =>
          old
      ...
      queryClient.setQueriesData<BlastTargetsResponse>(
        { queryKey: ["blast-targets", campaignId] },
        (old) => updateBlastTargetStatusInResponse(old, targetId, status),
      );

      return { previousTarget, previousLists };
    },
    ...
    onSuccess: (updated, status) => {
      queryClient.setQueryData(["blast-target", campaignId, targetId], updated);
      queryClient.setQueriesData<BlastTargetsResponse>(
        { queryKey: ["blast-targets", campaignId] },
        (old) => updateBlastTargetStatusInResponse(old, updated.id, updated.status),
      );
      toast.success(
        status === "ACTIVE"
          ? "Blast target kembali active."
          : status === "PAUSED"
            ? "Blast target dipause."
            : "Blast target diarchive.",
      );
    },
    onSettled: () => {
      queryClient.invalidateQueries({
  ...
  const metrics = useMemo(() => {
    const list = attempts ?? [];
    const campaignReportsForTarget =
      campaignReports?.data.filter(
        (report) => report.blastTargetId === targetId,
      ) ?? [];
    const reportsForTarget: BlastReport[] = campaignReportsForTarget.length
      ? campaignReportsForTarget
      : list
          .map((attempt) => attempt.report)
          .filter((report): report is BlastReport => Boolean(report));
    const completed = list.filter((attempt) => attempt.status === "COMPLETED");
    const engagement = reportsForTarget.reduce(
      (sum, report) =>
        sum +
        (report.likes ?? 0) +
        (report.comments ?? 0) +
    ...
    const views = reportsForTarget.reduce(
  ...
  if (isError) {
    return (
      <EmptyState
        ...
        title="Gagal memuat blast target"
  ...
  if (!target) {
    return (
      <EmptyState
        icon={<Target size={48} />}
        title="Blast target tidak ditemukan"
        description="Target mungkin sudah dihapus atau Anda tidak memiliki akses."
      />
    );
  }
  ...
  const currentAttempt =
    attempts?.find(
      (attempt) => attempt.status === "KEPT" || attempt.status === "AVAILABLE",
    ) ?? attempts?.[0];
      ...
      <div>
        <CampaignShell campaign={campaign} campaignId={campaignId}>
          <div className="section-heading-row">
            <div>
              <Link
                href={`/campaigns/${campaignId}/blast-links`}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: "0.35rem",
              ...
              <p className="section-subtitle">
                Trace current attempt ownership, proof, reports, and reblast
                history for this target post.
              </p>
            </div>
            {isAdmin && (
              <div className="campaign-hero-actions">
                ...
                <Button
                  ...
                  disabled={!canReblast || actionLoading}
                  onClick={() => setReblastOpen(true)}
                >
                  Reblast
                </Button>
                {isPaused ? (
                  ...
                  <Button
                    ...
                    loading={
                      statusMutation.isPending &&
                      statusMutation.variables === "PAUSED"
                    }
                    icon={<PauseCircle size={14} />}
                    disabled={!canReblast || actionLoading}
                ...
                {attemptsLoading ? (
                  ...
                  >
                    ...
                    <div className="muted-meta">
                      Create a first attempt to make this target available in
                      Blast Queue.
                    </div>
                  </div>
                )}
              ...
              >
                ...
                >
                  <div>
                    ...
                    <h3 style={{ fontSize: "1rem", fontWeight: 850 }}>
                      Blast Reports
                    </h3>
                  </div>
                  <Link
                ...
                {metrics.reports.length ? (
                        ...
                        <div key={report.id} className="report-row">
                          ...
                          >
                            <span style={{ color: "var(--cyan)" }}>
                              <Eye size={12} /> {formatNumber(report.views)}
                            </span>
                            <span style={{ color: "#f43f5e" }}>
                              <Heart size={12} /> {formatNumber(report.likes)}
                            </span>
                            <span style={{ color: "var(--status-kept)" }}>
                              <MessageCircle size={12} />{" "}
                              {formatNumber(report.comments)}
                            </span>
                            <span style={{ color: "var(--status-active)" }}>
                              <Share2 size={12} /> {formatNumber(engagement)}
                            </span>
                          </div>
                          <a
                  ...
                  >
                    ...
                    <div className="muted-meta">
                      BlastReport is created only after a Buzzer completes their
                      kept attempt.
                    </div>
                  </div>
                )}
                ...
                <div className="helper-block">
                  ...
                  <div style={{ display: "grid", gap: "0.75rem" }}>
                    ...
                    <div>
                      ...
                      <div>
                        {currentAttempt?.keepExpiresAt
                          ? formatDateTime(currentAttempt.keepExpiresAt)
                          : "-"}
                      </div>
                    </div>
                  </div>
              ...
              <section className="helper-panel" style={{ position: "static" }}>
                <div className="helper-block">
                  ...
                  <p className="muted-meta" style={{ lineHeight: 1.7 }}>
                    Reblast creates a new BlastAttempt with the next attempt
                    number. It does not overwrite old attempt history or
                    reports.
                  </p>
                </div>
              </section>
          ...
          <Modal
            ...
            footer={
              <>
                <Button variant="ghost" onClick={() => setReblastOpen(false)}>
                  Cancel
                </Button>
                <Button
                  loading={reblastMutation.isPending}
                  disabled={!canReblast}
                  onClick={() => reblastMutation.mutate()}
                  icon={<RefreshCw size={14} />}
                >
                  Create Attempt
                </Button>
            ...
            <div style={{ display: "grid", gap: "1rem" }}>
              <div className="blast-info-banner" style={{ marginBottom: 0 }}>
                ...
                <div>
                  ...
                  <span>
                    {" "}
                    A new AVAILABLE attempt will be opened for campaign Buzzer
                    members.
                  </span>
                  {!canReblast && (
                    <span> Target harus ACTIVE sebelum reblast.</span>
                  )}
                </div>
              </div>
